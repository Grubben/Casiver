...
#include "callImgui.hpp"

void textAlignRight(const std::string text)
{
    // std::string text = "1";
...
void textAlignCenter(const std::string text)
{
    // std::string text = "1";
    auto txtsize = ImGui::CalcTextSize(text.c_str()).x;

    auto posX = (ImGui::GetCursorPosX() + ImGui::GetColumnWidth() / 2 - txtsize / 2 
        - ImGui::GetScrollX() - 2 * ImGui::GetStyle().ItemSpacing.x);
    if(posX > ImGui::GetCursorPosX())
    ImGui::SetCursorPosX(posX);
    ImGui::Text("%s", text.c_str());
...
int main(int, char**)
{
    ...
    while (!done)
    {
		...
		ImGui::Begin("Table With Header");
        static ImGuiTableFlags flags = ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_ScrollY | ImGuiTableFlags_HighlightHoveredColumn | ImGuiTableFlags_NoHostExtendX;// | ImGuiTableFlags_Resizable;
        // PushStyleCompact();
        ...
        if (ImGui::BeginTable("table1", 3, flags))//, outer_size))
        {
            ...
            for (int row = 0; row < 60000; row++)
            {
                ...
                for (int column = 0; column < 3; column++)
                {
                    ...
                    // textAlignRight("Hello " + std::to_string(column) + std::to_string(row));
                    textAlignCenter("Hello " + std::to_string(column) + std::to_string(row));
                }
            }
            ImGui::EndTable();
        }
